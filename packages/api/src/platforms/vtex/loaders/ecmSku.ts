// this file should be virtually identical to the ./sku.ts loader in this same directory,
// except it should use the `ecmProductDetail` service method.
...
export const getEcmSkuLoader = (_: Options, clients: Clients) => {
  const loader = async (keys: readonly string[]) => {
    ...
    const { products } = await clients.search.ecmProductDetail({
